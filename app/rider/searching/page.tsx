...
import DriverMessagePopup from "@/components/popup"
...
export default function SearchingPage() {
  ...
  useEffect(() => {
    ...
    // Simulate ride searching and popup
    const popupTimer = setTimeout(() => setIsPopupOpen(true), 10000)
    const redirectTimer = setTimeout(() => router.push("/rider/tracking"), 20000)
  ...
  return (
    <div className="flex min-h-screen flex-col bg-white">
      ...
      <main className="flex flex-1 flex-col items-center justify-center p-4">
        <div className="mb-6 h-16 w-16 animate-spin rounded-full border-4 border-black border-t-white" />
        ...
        <div className="mt-8 w-full max-w-md rounded-none border-2 border-black p-4">
          ...
          <div className="space-y-2 text-sm">
            <div><span className="font-medium">{t("searching.from")}:</span> {localStorage.getItem("pickupLocation") || ""}</div>
            <div><span className="font-medium">{t("searching.to")}:</span> {localStorage.getItem("dropoffLocation") || ""}</div>
            <div><span className="font-medium">{t("searching.vehicle")}:</span> {localStorage.getItem("selectedVehicle") || ""}</div>
          </div>
        </div>
